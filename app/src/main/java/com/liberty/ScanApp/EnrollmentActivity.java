...
public class EnrollmentActivity extends Activity
{
	...
	public void onCreate(Bundle savedInstanceState)
	{
		...
		{
			Log.w("MainActivity", "error during init of reader");
			m_deviceName = "";
			onBackPressed();
			return;
		}
		...
		new Thread(new Runnable()
		{
			...
			public void run()
			{
				try 
				{
				    if (m_current_fmds_count > 0 && m_success){
				        onBackPressed();
                    }
                    else{
				...
				catch (Exception e)
				{
					if(!m_reset)
					{
						Log.w("MainActivity", "error during capture");
						m_deviceName = "";
						onBackPressed();
					}
				}
	...
	public void onBackPressed()
	{
		...
		{
			Log.w("MainActivity", "error during reader shutdown");
		}
		Intent i = new Intent();
	...
	implements Engine.EnrollmentCallback
	{
		...
		public PreEnrollmentFmd GetFmd(Format format)
		{
			...
			while (!m_reset)
			{
				...
				{
					Log.w("MainActivity", "error during capture: " + e.toString());
					m_deviceName = "";
					onBackPressed();
				}
				...
				catch (Exception e)
				{
					...
					Log.w("MainActivity", "Engine error: " + e.toString());
				}
			}
			...
			if (m_enrollment_fmd != null || m_current_fmds_count == 0)
			{
				if (!m_first)
				{
					...
					{
                        byte[] m_data = m_enrollment_fmd.getData(); // enrollment fmd to be passed to LineApp
				...
				m_textString = "Place your index finger on the reader. (" + Integer.toString(m_current_fmds_count) + "/5)";
