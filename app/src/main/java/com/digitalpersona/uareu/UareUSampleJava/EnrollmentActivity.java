			...
			if (m_enrollment_fmd != null || m_current_fmds_count == 0)
			{
				if (!m_first)
				{
					if (m_text_conclusionString.length() == 0)
					{
						...
						m_text_conclusionString = m_success ? "Enrollment template created, " + m_enrollment_fmd : "Enrollment template failed. Please try again";

						// open LineApp (com.lineapp)
                        //Activity currentActivity = getCurrentActivity();
                        //currentActivity.startActivity(new Intent("com.lineapp"));
                        Context applContext = getApplicationContext();
